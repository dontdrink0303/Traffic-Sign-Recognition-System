...
#     def __init__(self, c1, c2, shortcut=True, g=1, e=0.5, ratio=16):  # ch_in, ch_out
#         super().__init__()
#         c_ = int(c2 * e)  # hidden channels
#         self.cv1 = Conv(c1, c_, 1, 1)
#         self.cv2 = Conv(c_, c2, 3, 1, g=g)
...
#         self.l1 = nn.Linear(c2, c2 // ratio, bias=False)
#         self.relu = nn.ReLU(inplace=True)
#         self.l2 = nn.Linear(c2 // ratio, c2, bias=False)
#         self.sig = nn.Sigmoid()

#     def forward(self, x):
#         x1 = self.cv2(self.cv1(x))  # 通常Conv之后SE
#         b, c, _, _ = x1.size()
...
#         c_ = int(c2 * e)
#         self.m = nn.Sequential(*(SEBottleneck(c_, c_, shortcut, g, e) for _ in range(n)))
...
class CBAMBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        ca_weight = self.conv_ca(avg_out)
        x = x * ca_weight

        # --- Spatial Attention ---
        avg = torch.mean(x, dim=1, keepdim=True)
        max_ = torch.amax(x, dim=1, keepdim=True)  # 替代 max_pool，兼容性更好
        sa_weight = self.conv_sa(torch.cat([avg, max_], dim=1))
        x = x * sa_weight
